          ...
          <div className="flex flex-wrap gap-2 mb-4">
            ...
            {deal.responseStatus && (
              <span className={`inline-flex items-center gap-2 px-4 py-1.5 rounded-full text-sm font-semibold border backdrop-blur ${
                ...
                {/* Display-only relabel for direct-enrolment students — "Waiting
                    on Agent" doesn't make sense when there's no agent involved.
                    The underlying deal.responseStatus value (and the color
                    logic just above, which checks it) is completely unchanged;
                    this only swaps the text actually shown on screen. */}
                {deal.responseStatus === "Waiting on Agent" ? "Waiting on Student" : deal.responseStatus}
              </span>
            )}
            {deal.studentId && (
              <span className="inline-flex items-center gap-2 bg-white/15 border border-white/25 px-4 py-1.5 rounded-full text-sm font-semibold">
