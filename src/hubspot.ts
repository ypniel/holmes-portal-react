...
// ── Fetch Files ───────────────────────────────────────────────────────────────
const CRM_FILE_UPLOAD_PROPS = Array.from({ length: 10 }, (_, i) => `file_upload_${i + 1}`)

function cleanFileName(name: string, extension?: string): string {
  let cleaned = (name || "Document").trim()
...
function extractFileIds(value: unknown): string[] {
  ...
  const addFromString = (input: string) => {
    ...
    // HubSpot sometimes stores file upload values as JSON, semicolon-separated IDs,
    // preview URLs, proxy URLs, or mixed text containing one or more file IDs.
    for (const match of text.matchAll(/(?:fileId=|\/file\/|\/files\/|files\/v3\/files\/|files\/)\s*(\d+)/gi)) {
      ids.add(match[1])
    }
  ...
  if (Array.isArray(value)) {
    value.forEach(item => {
      if (typeof item === "string" || typeof item === "number") addFromString(String(item))
      else if (item && typeof item === "object") {
        const obj = item as Record<string, unknown>
        ;[obj.id, obj.fileId, obj.file_id, obj.value, obj.url].forEach(v => {
          if (typeof v === "string" || typeof v === "number") addFromString(String(v))
        })
      }
    })
    return [...ids]
  ...
  try {
    ...
    extractFileIds(parsed).forEach(id => ids.add(id))
  } catch {}

  raw.split(/[;,\n]/).forEach(addFromString)
  addFromString(raw)
  return [...ids]
}

async function fetchFileMetadata(fileId: string, createdAt?: number): Promise<FileItem> {
  try {
    const fileData = await hsFetch(`/files/v3/files/${fileId}`)
    const name = cleanFileName(fileData.name || fileData.path?.split("/").pop() || "Document", fileData.extension)
    return {
      name,
      id: fileId,
      url: `/.netlify/functions/hubspot?download=true&fileId=${fileId}`,
      createdAt: createdAt || (fileData.createdAt ? new Date(fileData.createdAt).getTime() : undefined),
    }
  } catch {
    try {
      ...
      return { name: "Document", id: fileId, url: `/.netlify/functions/hubspot?download=true&fileId=${fileId}`, createdAt }
    }
  }
}
...
export async function fetchFiles(dealId: string): Promise<FileItem[]> {
  try {
    ...
    // Method 1 — files submitted through HubSpot form CRM file properties.
    // These usually live in /crm-properties-file-values/ and may be HIDDEN_SENSITIVE.
    try {
      const deal = await hsFetch(`/crm/v3/objects/deals/${dealId}?properties=${CRM_FILE_UPLOAD_PROPS.join(",")}`)
      const props = deal.properties || {}
      const crmFileIds = new Set<string>()

      for (const prop of CRM_FILE_UPLOAD_PROPS) {
        extractFileIds(props[prop]).forEach(id => crmFileIds.add(id))
      }

      const crmFiles = await Promise.all([...crmFileIds].map(id => fetchFileMetadata(id)))
      files.push(...crmFiles)
    } catch {}

    // Method 2 — existing portal/File Manager repository files attached to engagements.
    // This preserves the current Documents layout and upload behaviour.
    ...
    for (const eng of data.results || []) {
      ...
      const createdAt = eng.engagement?.createdAt

      // Extract file IDs from HTML links saved in engagement notes.
      const linkMatches = [...body.matchAll(/<a[^>]*href="([^"]+)"[^>]*>([^<]+)<\/a>/g)]
      for (const match of linkMatches) {
        const hrefUrl = match[1]
        let name = cleanFileName(match[2].trim())
        if (!name) continue

        const fileIdMatch = hrefUrl.match(/\/files\/(\d+)\//) || hrefUrl.match(/fileId=(\d+)/) || hrefUrl.match(/\/file\/(\d+)\//)
        if (fileIdMatch) {
          const fileId = fileIdMatch[1]
          if (files.find(f => f.id === fileId)) continue
          files.push({ name, id: fileId, url: `/.netlify/functions/hubspot?download=true&fileId=${fileId}`, createdAt })
        } else if (hrefUrl.includes("hubspotusercontent") && !files.find(f => f.url === hrefUrl)) {
          files.push({ name, id: hrefUrl, url: hrefUrl, createdAt })
        }
      }

      // Attachment IDs → fetch metadata in parallel.
      const newAtts = (eng.attachments || [])
        .filter((att: any) => att.id && att.id !== 0 && !files.find(f => f.id === String(att.id)))
        .map((att: any) => String(att.id))

      const metaResults = await Promise.all(newAtts.map((attId: string) => fetchFileMetadata(attId, createdAt)))
