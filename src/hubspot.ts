...
const FILE_UPLOAD_PROPS = Array.from({ length: 10 }, (_, i) => `file_upload_${i + 1}`)
...
export async function fetchNotes(dealId: string): Promise<Note[]> {
  ...
  try {
    ...
    for (const e of eng.results || []) {
      ...
      let body = e.metadata?.body || e.metadata?.html || ""
...
export async function createNote(
  ...
  try {
    ...
    const engBody = JSON.stringify({
      ...
      metadata: {
        from: {
          email: "portal@holmes.edu.au",
          firstName: authorName || "Agent",
        },
...
export async function fetchDealCompany(dealId: string): Promise<Company | null> {
  try {
    ...
    let companyIds: string[] = (assoc.results || [])
      .slice(0, 1)
      .map((a: any) => String(a.toObjectId || a.id))

    if (!companyIds.length) {
      const assocV3 = await hsFetch(`/crm/v3/objects/deals/${dealId}/associations/companies`)
      const v3Ids = (assocV3.results || [])
        .slice(0, 1)
...
export async function fetchAgentByEmail(email: string): Promise<{
  ...
  try {
    ...
    return {
      ...
      contactName: company.properties?.contact_person_name || `${contact.properties?.firstname || ""} ${contact.properties?.lastname || ""}`.trim(),
...
function extractFileIds(value: any): string[] {
  if (!value) return []

  const text = String(value)
  const ids = new Set<string>()

  const directIdMatches = text.match(/\b\d{9,}\b/g)
  if (directIdMatches) {
    for (const id of directIdMatches) ids.add(id)
  }

  const previewMatches = [...text.matchAll(/\/file\/(\d+)/g)]
  for (const match of previewMatches) ids.add(match[1])

  const fileIdMatches = [...text.matchAll(/fileId=(\d+)/g)]
  for (const match of fileIdMatches) ids.add(match[1])
...
async function fetchFileMetadata(fileId: string): Promise<FileItem | null> {
  try {
    let fileData: any

    try {
      fileData = await hsFetch(`/files/v3/files/${fileId}`)
    } catch {
      fileData = await hsFetch(`/filemanager/api/v3/files/${fileId}`)
    }
    ...
    return {
      name,
      id: fileId,
      url: `/.netlify/functions/download-file?fileId=${fileId}`,
      createdAt: fileData.createdAt ? new Date(fileData.createdAt).getTime() : Date.now(),
    }
  } catch {
    return {
      name: "Document",
...
export async function fetchFiles(dealId: string): Promise<FileItem[]> {
  try {
    ...
    // Method 0: CRM form upload properties, file_upload_1 to file_upload_10.
    try {
      const deal = await hsFetch(`/crm/v3/objects/deals/${dealId}?properties=${FILE_UPLOAD_PROPS.join(",")}`)
      const props = deal.properties || {}

      const crmFileIds = new Set<string>()

      for (const propName of FILE_UPLOAD_PROPS) {
        for (const fileId of extractFileIds(props[propName])) {
          crmFileIds.add(fileId)
        }
      }

      const crmFiles = await Promise.all([...crmFileIds].map(fetchFileMetadata))

      for (const file of crmFiles) {
        if (file && !files.find(f => f.id === file.id)) {
          files.push(file)
        }
      }
    } catch {}

    // Existing repository / engagement attachment logic.
    ...
    for (const eng of data.results || []) {
      ...
      const metaResults = await Promise.all(
        newAtts.map(async (attId: string) => {
          const meta = await fetchFileMetadata(attId)

          return {
            ...(meta || {
              name: "Document",
              id: attId,
              url: `/.netlify/functions/download-file?fileId=${attId}`,
            }),
            createdAt: eng.engagement?.createdAt,
          }
        })
